...
function checkPlayerAmount (amount, inputValue) {
  if (Number(inputValue) === 0 || Number(inputValue) / amount == 1) return false; 
  else return true; 
}

function step2 () {

  const playerAmount = checkPlayerAmount(tournament.amountOfPlayers, range.value); 
...
function step3 () {
  ...
  if (tournament.amountOfPlayers == tournament.players.length) {
  ...
  } else if (tournament.players.length > tournament.amountOfPlayers) {
    console.log(`You have added to many players.. You need to remove ${tournament.players.length - tournament.amountOfPlayers} players`); 
  } else {
    console.log(`You need to add ${tournament.amountOfPlayers - tournament.players.length} more players`); 
  }
...
function collectPlayerInfo () {
  ...
  rowsValue.forEach((player, index) => { 
    if (player.length > 0) {
...
function checkSegment (tier) {
  ...
  switch(tier) {
    case "top": 
      if (tier === "top") return playerTiers.top;  
      else if (tier === "middle") return playerTiers.middle; 
      else if (tier === "bottom") return playerTiers.bottom; 
    break; 
    case "middle": 
      if (tier === "middle") return playerTiers.middle;  
      else if (tier === "bottom") return playerTiers.bottom; 
      else if (tier === "top") return playerTiers.top;
    break; 
    case "bottom":  
      if (tier === "bottom") return playerTiers.bottom;  
      else if (tier === "top") return playerTiers.top; 
      else if (tier === "middle") return playerTiers.middle;     
    break;   
    default:
